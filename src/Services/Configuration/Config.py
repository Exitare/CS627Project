...
def validate_config():
    ...
    if not Config.FILE_MEMORY_VAR_SUMMARY:
        logging.error(f"Please specify a valid name for FILE_MEMORY_VAR_SUMMARY ")
        sys.exit()

    if not Config.FILE_MEMORY_MEAN_SUMMARY:
        logging.error(f"Please specify a valid name for FILE_MEMORY_MEAN_SUMMARY")
        sys.exit()

    if not Config.FILE_RUNTIME_VAR_SUMMARY:
        logging.error(f"Please specify a valid name for FILE_RUNTIME_VAR_SUMMARY")
        sys.exit()

    if not Config.FILE_RUNTIME_MEAN_SUMMARY:
        logging.error(f"Please specify a valid name for FILE_RUNTIME_MEAN_SUMMARY")
        sys.exit()

    if Config.K_FOLDS <= 3:
        logging.error(f"A negative or value below 3 for folds is invalid. Setting to 5...")
        Config.K_FOLDS = 5

    if Config.REPETITIONS <= 0:
        logging.warning(f"A negative or zero value for repetitions is invalid. Setting to 5...")
        Config.REPETITIONS = 5

    if Config.REPETITIONS <= 0:
        logging.warning(f"A negative or zero value for forest estimators is invalid. Setting to 12...")
