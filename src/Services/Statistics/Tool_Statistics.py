...
def __count_tool_statistics():
    ...
    for label in multi_tool_stats["Label"].unique():
        ...
        fig.savefig(
            Path.joinpath(Runtime_Folders.EVALUATION_DIRECTORY, f"{label}_test_score_distribution_multi_tools.jpg"),
            bbox_inches="tight")
        fig.clf()
        plt.close('all')
    ...
    for label in all_tools_stats["Label"].unique():
        ...
        fig.savefig(
            Path.joinpath(Runtime_Folders.EVALUATION_DIRECTORY, f"{label}_test_score_distribution_all_tools.jpg"),
            bbox_inches="tight")
        fig.clf()
        plt.close('all')
...
def __prediction_score_on_average_across_versions():
    ...
    try:
        ...
        fig.savefig(Path.joinpath(Runtime_Folders.EVALUATION_DIRECTORY,
                                  f"tools_test_score_on_average_score.jpg"),
                    bbox_inches="tight")
        fig.clf()
        plt.close('all')

        for label in tool_avg_scores["Label"].unique():
            data = tool_avg_scores[tool_avg_scores["Label"] == label]
            ...
            ax = sns.violinplot(data=data, x="Versions", y="Test Score")
            fig = ax.get_figure()
